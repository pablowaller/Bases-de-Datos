...
(	Id_Club smallint,
	Nombre char(30),
	Nrozona tinyint
	Constraint pk_Clubes Primary Key (Id_Club))
GO
...
(	Tipodoc	Char(3),
	...
	Categoria         Tinyint,
	Id_Club     	smallint
	Constraint pk_Jugadores Primary Key (Tipodoc, Nrodoc)
...
(	Id_Club 	smallint,
	...
	Constraint pk_PosCate184 Primary Key (Id_Club),
	Constraint Fk_PosCate184_Clubes Foreign Key (Id_club) References Clubes )
GO
...
(	Id_Club 	smallint,
	...
	Puntos		smallint null,
	Promedio 	dec(4,2) null,
	Constraint pk_PosCate284 Primary Key (Id_Club),
	Constraint Fk_PosCate284_Clubes Foreign Key (Id_club) References Clubes )
GO
...
(	Id_Club 	smallint,
	...
	Puntos		smallint null,
	Promedio 	dec(4,2) null,
	Constraint pk_PosCate185 Primary Key (Id_Club),
	Constraint Fk_PosCate185_Clubes Foreign Key (Id_club) References Clubes ) 
GO
...
(	Id_Club 	smallint,
	...
	Puntos		smallint null,
	Promedio 	dec(4,2) null,
	Constraint pk_PosCate285 Primary Key (Id_Club),
	Constraint Fk_PosCate285_Clubes Foreign Key (Id_club) References Clubes ) 
GO
...
(	Id_Club 	smallint,
	...
	Puntos		smallint null,
	Promedio 	dec(4,2) null,
	Constraint pk_General Primary Key (Id_Club),
	Constraint Fk_General_Clubes Foreign Key (Id_club) References Clubes )
GO
...
update poscate184 set promedio = convert(dec(4,2),puntos) / convert(dec(4,2),cantidad) 
...
update poscate284 set promedio = convert(dec(4,2),puntos) / convert(dec(4,2),cantidad) 
...
update poscate285 set promedio = convert(dec(4,2),puntos) / convert(dec(4,2),cantidad) 
...
update poscate185 set promedio = convert(dec(4,2),puntos) / convert(dec(4,2),cantidad) 
go
...
/*
Crear un procedimiento que cree una tabla de resumen de puntos para los clubes en un rango de números de fechas determinado. 

La estructura seria la siguientes:
En cada fila se indicará al club y en cada columna se indicarán los numero de 
fecha con el ordinal correspondiente a la fecha (primara, segunda, tercera, etc.) (35%)

En la intersección de las filas y las columnas deberá colocar la cantidad de puntos obtenidos 
por el club hasta esa fecha (si gano son 3 puntos, si empato 1 y si perdió ninguno) 
independientemente si juega de local o de visitante. (35%)

Verificar que el rango de números de fechas sea válido (debe tener al menos 4 números de fecha). 
Caso contrario enviar un mensaje y finalizar el procedimiento. (5%)
Deben existir resultados para ese rango de fechas indicado. Caso contrario enviar un mensaje 
y finalizar el procedimiento. (10%)

Devolver en una variable de output la cantidad de registros leídos para obtener el resultado. (10%)
El nombre de la tabla será Resultados_99_99 (por ejemplo, si puso 3 y 6 el nombre será RC Resultados 03_06). (5%)

*/
GO
CREATE OR ALTER PROCEDURE EJP2 
(@f INT, @f2 INT, @i INT OUTPUT, @ii INT OUTPUT)
AS
BEGIN
	...
	IF NOT EXISTS (SELECT * FROM Partidos WHERE NroFecha BETWEEN @f and @f2) BEGIN
		PRINT 'NO existen fechas'
		RETURN
	END
	...
	BEGIN	
		...
		SET @SQL = 'Fecha_'+CAST(@aux as NVARCHAR(MAX))
	...
	DECLARE @SQLCOMMAND NVARCHAR(MAX) = 'sp_rename '+CHAR(39)+'Tablax'+CHAR(39)+', '+CHAR(39)+'Resultados_'+CAST(@f as nvarchar(MAX))+'_'+CAST(@f2 as nvarchar(MAX))+CHAR(39)
...
EXEC EJP2 1,6,@i = @v1 OUTPUT,@ii = @v2 OUTPUT